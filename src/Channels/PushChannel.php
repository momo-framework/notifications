...
/**
 * Delivers notifications implementing {@see RoutesToPush} as device push alerts.
 */
final class PushChannel implements NotificationChannelInterface
{
    ...
    {
        if (! $notification instanceof RoutesToPush) {
            return false;
        }

        $token = $notifiable->routeNotificationFor($this->name());

        if ($token === null) {
            return false;
        }

        $this->transport->send($notification->toPush($notifiable), $token);
